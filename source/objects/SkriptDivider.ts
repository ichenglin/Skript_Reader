    ...
    public export_component(script: string, parent_object: SkriptObject): SkriptObject[] {
        ...
        for (let component_index = 0; component_index < this.components.length; component_index++) {
            ...
            if (loop_component.begin_index >= next_index + 1) {
                export_objects.push(divider_component_to_object(script, {begin_index: next_index, end_index: loop_component.begin_index - 1, component_type: parent_object.object_type}, parent_object.object_depth, parent_object.parent_types, true));
            }
        ...
        if (this.components.length > 0 && script.length >= next_index + 1) {
            export_objects.push(divider_component_to_object(script, {begin_index: next_index, end_index: script.length - 1, component_type: parent_object.object_type}, parent_object.object_depth, parent_object.parent_types, true));
        }
...
function divider_component_to_object(script: string, divider_component: SkriptDividerComponent, parent_depth: number, parent_types: SkriptType[], object_final: boolean): SkriptObject {
    const script_component = script.slice(divider_component.begin_index, divider_component.end_index + 1);
    return new SkriptObject(script_component, divider_component.component_type, parent_depth + 1, parent_types, object_final);
}